...
void Collision::buildBVH()
{
	for (int i = 0; i < total_obj_num; ++i) {
		if (i < tetrahedron_begin_obj_index) {
			obj_BVH[i].buildBVH(&(*cloth)[i].triangle_AABB);
		}
		else {
...
void Collision::collisionConstraint(int thread_No)
{
	...
	for (int cloth_No = 0; cloth_No < cloth->size(); ++cloth_No) {
		...
		neighbor_primitve = (*cloth)[cloth_No].vertex_neighbor_cloth_triangle.data();
...
void Collision::test()
{
	...
	for (int i = 0; i < (*cloth)[0].vertex_neighbor_cloth_triangle[0][0].size(); ++i) {
		//std::cout << (*cloth)[0].vertex_neighbor_cloth_triangle[0][0][i] << " ";
	}
	//std::cout << std::endl;
	...
	for (int i = 0; i < collider->size(); ++i) {
		...
		for (int k = 0; k < total_obj_num; ++k) {
			if (k < tetrahedron_begin_obj_index) {
				...
				compared_vertex_aabb = &(*tetrahedron)[obj_No].vertex_AABB;
...
void Collision::findObjTriangleAroundVertex(int thread_No)
{
	...
	for (int i = 0; i < total_obj_num; ++i) {
		if (i < tetrahedron_begin_obj_index) {
			...
			vertex_aabb = &(*tetrahedron)[obj_No].vertex_AABB;
		}
	...
	for (int i = 0; i < total_obj_num; ++i) {
		...
		for (int k = i; k < total_obj_num; ++k) {
			if (k < tetrahedron_begin_obj_index) {
				...
				compared_edge_aabb = &(*tetrahedron)[obj_No2].edge_AABB;
