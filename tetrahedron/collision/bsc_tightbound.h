...
#define BOUND_UNIT 0.00000000000000022204460492503136
	...
	inline int subdivide(floating<T> k0, floating<T> k1, floating<T> k2, floating<T> k3, floating<T>* g){
		...
		//k0, 
		//(1-t)*k0 + t*k1, 
		//(1-t)^2*k0 + 2*(1-t)*t*k1 + t^2*k2, 
		//(1-t)^3*k0 + 3*(1-t)^2*t*k1 + 3*(1-t)*t^2*k2 + t^3*k3
		...
		bool ee_test)
	{
		...
		//std::cout << "k0,k1,k2,k3 " << k0.v << " " << k1.v << " " << k2.v << " " << k3.v << std::endl;
		...
		//std::cout << "kk " <<ct<<" "<< kk0.v << " " << kk1.v << " " << kk2.v << std::endl;
		...
		//std::cout <<"root_nums "<< root_nums<< std::endl;
