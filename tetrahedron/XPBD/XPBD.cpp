...
void XPBD::computeEdgeLenthResidual()
{
	...
	for (unsigned int i = 0; i < cloth->size(); ++i) {
		...
		edge_vertex_index = mesh_struct[i]->edge_vertices.data();
...
void XPBD::computeResidual()
{
	...
	for (unsigned int i = 0; i < total_obj_num; ++i) {
		mass= mesh_struct[0]->mass.data();
		Sn_ = sn[i].data();
		v_p = vertex_position[i];
		residual_ = residual[i].data();
		vertex_size = mesh_struct[i]->vertex_position.size();
		for (unsigned int j = 0; j < vertex_size; ++j) {
			residual_[j][0] = mass[j] * (v_p[j][0] - Sn_[j][0]) / (sub_time_step * sub_time_step);
			residual_[j][1] = mass[j] * (v_p[j][1] - Sn_[j][1]) / (sub_time_step * sub_time_step);
			residual_[j][2] = mass[j] * (v_p[j][2] - Sn_[j][2]) / (sub_time_step * sub_time_step);
		}
	}
	...
	for (unsigned int i = 0; i < total_obj_num; ++i) {
		residual_ = residual[i].data();
		for (unsigned int j = 0; j < mesh_struct[i]->vertex_position.size(); ++j) {
...
void XPBD::solveBySecondOrderXPBD()
{
	...
	for (unsigned int sub_step = 0; sub_step < sub_step_num; ++sub_step) {
		...
		while (!convergeCondition(inner_iteration_number)) {
			//std::cout << mesh_struct[0]->vertex_position[7][0] << " " << mesh_struct[0]->vertex_position[7][1] << " " << mesh_struct[0]->vertex_position[7][2] << std::endl;
			//std::cout << sn[0][7][0] << " " << sn[0][7][1] << " " << sn[0][7][2] << std::endl;
			recordVertexPosition();
...
void XPBD::computeCurrentEnergy()
{
	//std::cout << "///" << std::endl;
	energy = 0.0;
...
double XPBD::computeCurrentEnergyEdgeLength()
{
	double energy = 0.0;
	unsigned int size;
	MeshStruct* mesh_struct_;
	...
	for (unsigned int i = 0; i < cloth->size(); ++i) {
		...
		stiffness = cloth->data()[i].length_stiffness;
		edge_vertex_index = mesh_struct_->edge_vertices.data();
		edge_length = mesh_struct_->edge_length.data();
		for (auto k = mesh_struct_->unconnected_edge_index.begin(); k < mesh_struct_->unconnected_edge_index.end(); ++k) {
			for (auto j = k->begin(); j < k->end(); ++j) {
				length_dis = sqrt(EDGE_LENGTH(vertex_pos[edge_vertex_index[(*j) << 1]].data(), vertex_pos[edge_vertex_index[((*j) << 1) + 1]].data()))- edge_length[*j];
				energy += stiffness * length_dis * length_dis;
			}
		}
	}
	return energy;
	...
	for (unsigned int i = 0; i < cloth->size(); ++i) {
		...
		//	for (auto j = k->begin(); j < k->end(); ++j) {
				...
				second_order_constraint.solveEdgeLengthConstraint(vertex_pos[edge_vertex_index[(j) << 1]].data(),
					...
					edge_vertex_index[(j) << 1], edge_vertex_index[((j) << 1) + 1]);
				//std::cout << vertex_pos[edge_vertex_index[j << 1]].data()[0]<<" "<< vertex_pos[edge_vertex_index[(j << 1) + 1]].data()[0]<<" "<< edge_vertex_index[j << 1] << " " << edge_vertex_index[(j << 1) + 1] << std::endl;		
				lambda_++;
			//}
		}
...
void XPBD::solveBendingConstraint()
{
	...
	//double energy_;
	for (unsigned int i = 0; i < cloth->size(); ++i) {
		if (mesh_struct[i]->vertex_position.size() < 3) {
...
double XPBD::computeInertialEnergy()
{
	double energy = 0.0;
	std::array<double, 3>* vertex_pos;
	std::array<double, 3>* sn_;
