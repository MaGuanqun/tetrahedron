...
double SecondOrderConstraint::solveEdgeLengthConstraint(double* p0, double* p1, const double rest_length)
{
	Vector3d n;
	SUB(n.data(), p0, p1);
	double n_norm = sqrt(DOT(n, n));
	double coe = (1.0 - rest_length / n_norm);
	...
	//memcpy(H.data() + 12, He.data() + 6, 24);
	//memcpy(H.data() + 33, He.data() + 6, 24);
	//LDLT<Matrix<double, 6, 6>> ldlt(H);
	//Matrix<double, 6, 1> delta_x = ldlt.solve(neg_grad);
	...
	//std::cout << delta_x.segment(0, 3) <<" "<< delta_x.segment(3, 3) << std::endl;
