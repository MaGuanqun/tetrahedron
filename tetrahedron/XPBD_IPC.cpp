	...
	Thread* thread, double* tolerance_ratio)
{
	...
	ipc.setForIPC(cloth, tetrahedron, collider,  floor, thread, tolerance_ratio,true);
...
void XPBD_IPC::reorganzieDataOfObjects()
{
	...
	record_first_collision_free_position.resize(total_obj_num);
	...
	for (unsigned int i = 0; i < cloth->size(); ++i) {
		...
		record_first_collision_free_position[i] = cloth->data()[i].mesh_struct.vertex_position;
	...
	for (unsigned int i = 0; i < tetrahedron->size(); ++i) {
		...
		record_first_collision_free_position[i + cloth->size()]=tetrahedron->data()[i].mesh_struct.vertex_position;
...
//WARM START
void XPBD_IPC::warmStart(double& ori_energy)
{
...
void XPBD_IPC::testEdge()
{
	int v0 = 224, v1 = 1189;
	int v2 = 54, v3 = 21;

	//rootParityTest(initial[0])
}

void XPBD_IPC::rootParityTest(double* v0_ini, double* v0_cu, double* v1_ini, double* v1_cu,
	double* v2_ini, double* v2_cu, double* v3_ini, double* v3_cu)
{
	Vec3d initial_pos_a = Vec3d(v0_ini[0], v0_ini[1], v0_ini[2]);
	Vec3d current_pos_a = Vec3d(v0_cu[0], v0_cu[1], v0_cu[2]);
	Vec3d initial_triangle_0_a = Vec3d(v1_ini[0], v1_ini[1], v1_ini[2]);
	Vec3d initial_triangle_1_a = Vec3d(v2_ini[0], v2_ini[1], v2_ini[2]);
	Vec3d initial_triangle_2_a = Vec3d(v3_ini[0], v3_ini[1], v3_ini[2]);
	Vec3d current_triangle_0_a = Vec3d(v1_cu[0], v1_cu[1], v1_cu[2]);
	Vec3d current_triangle_1_a = Vec3d(v2_cu[0], v2_cu[1], v2_cu[2]);
	Vec3d current_triangle_2_a = Vec3d(v3_cu[0], v3_cu[1], v3_cu[2]);
	...
	bool test2 = root_parity.edge_edge_collision();

	if (test2) {
		std::cout << "==error : two edge collide" << std::endl;
	}
			...
			for (int j = 0; j < 4; ++j) {
				if (vertex_index[k] <= vertex_index[j]) {
					...
					if (find_ != neo_hookean_hessian->end()) {
						...
						find_->second.order_in_constraint.emplace_back(3*((12*j)+k));
					}
					else {
						temp_store_.order_in_constraint[0] = i;
						temp_store_.order_in_constraint[1] = 3 * ((12 * j) + k);
...
//INVERSION_TEST
// 0 inital pos is the last itr collision free,  1 initial is the record pos, 2 first collision free pos for warmstart
void XPBD_IPC::inversionTest(int thread_No, int type)
{
	unsigned int start, end;
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		if (type == 0) {
			...
			ori_vertex_pos = record_first_collision_free_position[i + cloth->size()].data();
		}
		...
		for (auto j = start; j < end; ++j) {
			...
			if(inversionTest::TetInversionTest(ori_vertex_pos[tet_vertex[0]].data(), ori_vertex_pos[tet_vertex[1]].data(),
				...
				//if (inversion_time < 1e-7) {
				//	std::cout << "collision time too small " << i << " " << j << std::endl;
				//	double t;
				...
				//}
				//if (type == 1 && j == 7451) {
				//	std::cout <<"test inversion "<< inversion_time << std::endl;
				//}
				//if (inversion_time < 1e-7) {
				//	std::cout << "inversion time too small " << j<<" "<< inversion_time << std::endl;
				//}
				//if (collision_time < 0.1) {
				//	std::cout << "tet index " << j << " " << tet_vertex[0] << " " << tet_vertex[1] << " " << tet_vertex[2] << " " << tet_vertex[3]<<" "<< collision_time << std::endl;
				//	
				//	
				//	inversionTest::TetInversionTestTestCompare(ori_vertex_pos[tet_vertex[0]].data(), ori_vertex_pos[tet_vertex[1]].data(),
				//		ori_vertex_pos[tet_vertex[2]].data(), ori_vertex_pos[tet_vertex[3]].data(), vertex_pos[tet_vertex[0]].data(),
				//		vertex_pos[tet_vertex[1]].data(), vertex_pos[tet_vertex[2]].data(), vertex_pos[tet_vertex[3]].data(), &t);
				//}
			}
			...
			//if ( type == 1 && j == 2486) {
			//	for (int m = 0; m < 4; ++m) {
			//		std::cout << vertex_pos[tet_vertex[m]][0] << " " << vertex_pos[tet_vertex[m]][1] << " " << vertex_pos[tet_vertex[m]][2] << std::endl;
			//	}				
			//	if (inversionTest::TetInversionTestTest(ori_vertex_pos[tet_vertex[0]].data(), ori_vertex_pos[tet_vertex[1]].data(),
			//		ori_vertex_pos[tet_vertex[2]].data(), ori_vertex_pos[tet_vertex[3]].data(), vertex_pos[tet_vertex[0]].data(),
			//		vertex_pos[tet_vertex[1]].data(), vertex_pos[tet_vertex[2]].data(), vertex_pos[tet_vertex[3]].data(), &inversion_time)) {
			//		std::cout << inversion_time << std::endl;
			//		
			//		NeoHookean::testDeterminent(v0[0], v0[1], v0[2], v0[3],
			//			tet_A[0][j]);		
			//	}				
			//}
			//else {
			...
			//		for (int m = 0; m < 4; ++m) {
			//			std::cout << std::setprecision(10) << ori_vertex_pos[tet_vertex[m]][0] << " " << ori_vertex_pos[tet_vertex[m]][1] << " " << ori_vertex_pos[tet_vertex[m]][2] << std::endl;
			//		}
			//		for (int m = 0; m < 4; ++m) {
			//			std::cout <<std::setprecision(10)<< vertex_pos[tet_vertex[m]][0] << " " << vertex_pos[tet_vertex[m]][1] << " " << vertex_pos[tet_vertex[m]][2] << std::endl;
			//		}			
			//		for (int m = 0; m < 4; ++m) {
			//			std::cout << std::setprecision(10) << v0[m][0] << " " << v0[m][1] << " " << v0[m][2] << std::endl;
			//		}
			//		std::cout << "test finished "<<type<<" "<<t<<" "<< tet_vertex[0]<<" "<< tet_vertex[1]<<" "<< tet_vertex[2]<<" "<< tet_vertex[3]<< std::endl;
			//	}
			//	else {
			//		NeoHookean::testDeterminent(vertex_pos[tet_vertex[0]].data(),
			//			vertex_pos[tet_vertex[1]].data(), vertex_pos[tet_vertex[2]].data(), vertex_pos[tet_vertex[3]].data(), tet_A[0][j]);
			//		std::cout << " test inversion failed to detect " << type << std::endl;
			//	}			
			//}
		}
...
void XPBD_IPC::tetGrad(int thread_No)
{
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		for (int j = start; j < end; ++j)
		{
			NeoHookean::gradientMPM(vertex_pos[tet_indices_[j][0]].data(), vertex_pos[tet_indices_[j][1]].data(),
				vertex_pos[tet_indices_[j][2]].data(), vertex_pos[tet_indices_[j][3]].data(),
				A[j], volume[j], mu, lambda, grad);
...
bool XPBD_IPC::convergeCondition(unsigned int iteration_num, bool for_warm_start, double max_move_standard, unsigned int outer_max_iteration_number)
{
	...
	if (for_warm_start) {
		if (iteration_num < 1) {//max_iteration_number
			std::cout <<"warm start "<<  iteration_num << " " << max_displacement<<" "<< grad_max << std::endl;
			return false;
		}
	}
	else {
...
void XPBD_IPC::computeGradient(bool for_warm_start)
{
	for (int i = 0; i < total_thread_num; ++i) {
		memset(common_grad[i].data(), 0, 8 * common_grad[i].size());
	}
...
void XPBD_IPC::NeoHookeanTest()
{
	std::cout << "sum record " << std::endl;
	for (int i = 0; i < 12; ++i) {
		std::cout << common_grad[0][i] << " ";
	}
	std::cout << std::endl;

	for (int i = 0; i < 12; ++i) {
		for (int j = 0; j < 12; ++j) {
			std::cout << neo_hookean_hessian_record[0][12 * j + i] << " ";
		}
		std::cout << std::endl;
	}

	for (auto i = neo_hookean_hessian[0].begin(); i != neo_hookean_hessian[0].end(); ++i) {
		std::cout << i->first[0] << " " << i->first[1] << std::endl;
		for (int j = 0; j < 3; ++j) {
			for (int k = 0; k < 3; ++k) {
				std::cout << i->second.hessian[3 * k + j] << " ";
			}
			std::cout << std::endl;
		}
...
void XPBD_IPC::testVolume()
{
	for (int i = 0; i < tet_index_begin_per_thread[0][total_thread_num]; ++i) {
		double volume = getTetrahedronVolume(vertex_position[0][tet_indices[0][i][0]].data(), vertex_position[0][tet_indices[0][i][1]].data(), 
			vertex_position[0][tet_indices[0][i][2]].data(), vertex_position[0][tet_indices[0][i][3]].data());
		if (volume <= 0.0) {
			std::cout << "error  volume is negative "<<i << std::endl;
		}
	}
}


void XPBD_IPC::warmStartJacobi()
{
...
void XPBD_IPC::lineSearch(double& ori_energy)
{
	double current_energy = 0.0;
	if (perform_collision) {
...
void XPBD_IPC::lineSearchWarmStart(double& ori_energy, bool is_outer_itr, int use_previous_pair)
{
	...
	if (!is_outer_itr) {
		...
		if (collision->collision_time < 1.0) {
			thread->assignTask(collision, COLLISION_FREE_POSITION_ALL);
		}
	}
...
void XPBD_IPC::lineSearchColor(unsigned int color_No, double ori_energy)
{
	double current_energy = 0.0;
	...
	if (perform_collision) {
		...
		if (current_energy > energy_converge_standard) {
			...
			if (current_energy > ori_energy) {
				if (record_collision_time < 1e-6) {
					std::cout << "original record_collision_time is too small " << color_No << " " << ori_energy << " " << current_energy << std::endl;
				}
			}
			collision->collision_time = 0.5;
			//line search, make sure energy is decrease
			...
			while (current_energy > ori_energy)
			{
				...
				itr_num++;
				if (itr_num > 6) {
					//std::cout << "record_collision_time is too small " << color_No << " " << ori_energy << " " << current_energy << std::endl;
					break;
				}
				//if (record_collision_time < 1e-6) {
				//	
		...
		//else {
		//	std::cout << "color actual collision time " << color_No << " " << collision->collision_time << std::endl;
		//}
...
//UPDATE_POSITION_AVERAGE_EXCEPT_THIS_COLOR
void XPBD_IPC::updatePositionAverageExceptThisColor(int thread_No)
{
	int start, end;
	std::vector<std::array<double, 3>>* record_position;
	std::vector<int>* record_position_num;
...
void XPBD_IPC::updatePositionAverageWarmStart(int thread_No)
{
	...
	for (int i = 0; i < total_obj_num; ++i) {
		...
		for (int j = start; j < end; ++j) {
			...
			vertex_num = 0;
			for (int k = 0; k < total_thread_num; ++k) {
				if (record_position_num[k][j] == -1) {
					record_position_num[0][j] = -1;
					vertex_num = -1;
					break;
				}
				if (record_position_num[k][j]) {
...
void XPBD_IPC::allPairCollisionInversionTime()
{
	...
	//std::cout << "color co " << collision->collision_time << std::endl;
	thread->assignTask(this, INVERSION_TEST,1);
	for (int i = 0; i < total_thread_num; ++i) {
		if (collision->collision_time_thread[i] < collision->collision_time) {
			collision->collision_time = collision->collision_time_thread[i];
		}
...
void XPBD_IPC::newtonCDTetBlockAGroup(int thread_No, int color)
{
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {			
			...
			for (int c = 0; c < max_tet_color_num; ++c) {
				tet_group = tet_color_groups[i]->data()[color_group_index][c].data();
				end = tetrahedron->data()[i].mesh_struct.tet_in_a_group_start_per_thread_groups[color_group_index][c][thread_No + 1];
				start = tetrahedron->data()[i].mesh_struct.tet_in_a_group_start_per_thread_groups[color_group_index][c][thread_No];
				if (c == color) {
...
void XPBD_IPC::warmstart_solveBlockWithPair(double dt,
	...
	double* floor_map, double& max_dis, bool direct_update)
{
	...
	LLT <MatrixXd> linear(Hessian);
	VectorXd result = linear.solve(grad);
	double dis = result.lpNorm<Eigen::Infinity>();
	if (dis > max_dis) {
		max_dis = dis;
	}
	...
	if (!direct_update) {
		for (int i = 0; i < unfixed_num; i += 2) {
			vertex_index_ = unfixed_pair_vertex_index[i + 1];
			obj_index_ = unfixed_pair_vertex_index[i];
			if (record_vertex_num[obj_index_][vertex_index_] != -1) {
		...
		for (int i = 0; i < unfixed_num; i += 2) {
			...
			obj_index_ = unfixed_pair_vertex_index[i];
			SUB_(vertex_position[obj_index_][vertex_index_], (result.data() + 3 * (i >> 1)));
			record_vertex_num[obj_index_][vertex_index_] = -1;
		}
	}
	...
	double* floor_map, bool only_solve_collision_pair, double& max_dis)
{
	...
	dis = result.lpNorm<Eigen::Infinity>();
	if (dis > max_dis) {
		max_dis = dis;
	}
...
void XPBD_IPC::solveCollisionBlockWarmStart(int thread_No, unsigned int type, unsigned int index, double dt,
	std::array<double, 3>** record_vertex_position, int** record_vertex_num, bool direct_update)
{
	...
	{
	case 0:
		pair = collision->collision_pair_sum[type]->data() + (index << 2);
		getVTUnfixedPairIndex(unfixed_pair_vertex_index, unfixed_num, pair[0], pair[1], pair[2], pair[3]);
		break;
	case 1:
		pair = collision->collision_pair_sum[type]->data() + (index << 2);
		getEEUnfixedPairIndex(unfixed_pair_vertex_index, unfixed_num, pair[0], pair[1], pair[2], pair[3]);
		break;
	case 4:
		pair = collision->collision_pair_sum[type]->data() + (index << 1);
		if (!(*is_vertex_fixed[pair[0]])[pair[1]]) {
			unfixed_pair_vertex_index[0] = pair[0];
			unfixed_pair_vertex_index[1] = pair[1];
			unfixed_num = 2;
		}
		break;
		//e
	case 3: {
		pair = collision->collision_pair_sum[type]->data() + (index << 1);
		unsigned int* edge_vertex_0 = edge_vertices[pair[0]] + (pair[1] << 1);
		if ((!(*is_vertex_fixed[pair[0]])[*edge_vertex_0])) {
			unfixed_pair_vertex_index[unfixed_num] = pair[0];
			unfixed_pair_vertex_index[unfixed_num + 1] = *edge_vertex_0;
			unfixed_num += 2;
		}
		if ((!(*is_vertex_fixed[pair[0]])[edge_vertex_0[1]])) {
			unfixed_pair_vertex_index[unfixed_num] = pair[0];
			unfixed_pair_vertex_index[unfixed_num + 1] = edge_vertex_0[1];
			unfixed_num += 2;
		}
	...
	case 2:
		pair = collision->collision_pair_sum[type]->data() + (index << 1);
		int* tri_indices = triangle_indices[pair[0]][pair[1]].data();
		for (unsigned int i = 0; i < 3; ++i) {
			if (!(*is_vertex_fixed[pair[0]])[tri_indices[i]]) {
				unfixed_pair_vertex_index[unfixed_num] = pair[0];
				unfixed_pair_vertex_index[unfixed_num + 1] = tri_indices[i];
				unfixed_num += 2;
			}
	...
	warmstart_solveBlockWithPair(dt, record_vertex_position, record_vertex_num, unfixed_pair_vertex_index, unfixed_num, common_grad[0].data(),
		common_hessian, floor_hessian.data(), max_dis_record[thread_No], direct_update);
	...
	int* record_vertex_num, unsigned int prefix_sum_vetex_obj, double* floor_map, double& max_dis, std::array<double, 3>* collision_free_pos)//
{
	...
	//std::cout << "solve grad " << grad.transpose() << std::endl;
	//std::cout << Hessian << std::endl;
	...
	//	if (abs(result[j]) > 0.3) {
	//		std::cout << result.transpose() << std::endl;
	//		std::cout << "solveTetBlockCollision() move so large " << inner_iteration_number << " " << tet_index << " " << tet_actual_unfixed_vertex_indices[0] << " " << tet_actual_unfixed_vertex_indices[1] << " " << tet_actual_unfixed_vertex_indices[2] << " " << tet_actual_unfixed_vertex_indices[3] << std::endl;
	//		for (int i = 0; i < unfixed_vertex_num; ++i) {
	//			vertex_index = tet_actual_unfixed_vertex_indices[i];
...
void XPBD_IPC::addExternalForce(double* neighbor_vertex_force_direction, std::vector<double>& coe, std::vector<int>& neighbor_vertex, int obj_No)
{
	...
	ipc.addExternalForce(neighbor_vertex_force_direction, coe, neighbor_vertex, obj_No);
}
...
//COLLISION_FREE_POSITION_, type=1, warmstart, type=0 record_collision_free_vertex_position, 
void XPBD_IPC::computeCollisionFreePosition(int thread_No, unsigned int type)
{
	double collision_time = collision->collision_time;
	...
	for (unsigned int i = 0; i <total_obj_num; ++i) {
		...
		if (type == 0) {
			...
			q_pre = record_first_collision_free_position[i].data();
		}
		for (unsigned int j = vertex_index_begin_per_thread[i][thread_No]; j < index_end; ++j) {
			q_end[j][0] = q_pre[j][0] + collision_time * (q_end[j][0] - q_pre[j][0]);
			q_end[j][1] = q_pre[j][1] + collision_time * (q_end[j][1] - q_pre[j][1]);
