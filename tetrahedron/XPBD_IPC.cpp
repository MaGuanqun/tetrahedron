...
bool XPBD_IPC::checkMaxDisplacement()
{
	...
	//std::cout << "max in checkMaxDisplacement" << max << std::endl;
...
void XPBD_IPC::tetGradForColor(int thread_No, unsigned int color_No)
{
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		for (auto j = start; j < end; ++j) {
			...
			memcpy(store_tet_arap_grad.data() + 12 * (prefix_sum_start + tet_in_a_group[j]), grad.data(), 96);//			
			is_tet_arap_grad_compute[prefix_sum_start + tet_in_a_group[j]] = true;
		}
		...
		for (auto j = start; j < end; ++j) {
			second_order_constraint.setARAPGrad(grad, vertex_pos, stiffness, A[tet_around_a_group[j]],// ,
				volume[tet_around_a_group[j]], tet_indices_[tet_around_a_group[j]].data()); //,
			memcpy(store_tet_arap_grad.data() + 12 * (prefix_sum_start + tet_around_a_group[j]), grad.data(), 96);//
			is_tet_arap_grad_compute[prefix_sum_start + tet_around_a_group[j]] = true;
		}
	}
...
void XPBD_IPC::setCollisionPairTetNeighborGrad(int tet_obj_No, int start, int end, unsigned int* tet_involved, unsigned int prefix_sum_start)
{
	...
	for (int i = start; i < end; ++i) {
		for (auto j = tet_tet[i].begin(); j < tet_tet[i].end(); ++j) {
			if (!is_tet_arap_grad_compute[prefix_sum_start +*j]) {
				is_tet_arap_grad_compute[prefix_sum_start + *j] = true;
...
void XPBD_IPC::setCollisionPairTetGrad(int tet_obj_No, int start, int end, unsigned int* tet_involved, unsigned int prefix_sum_start)
{
	double stiffness = 0.5 * tetrahedron->data()[0].ARAP_stiffness;
	std::array<double, 3>* vertex_pos = vertex_position[tet_obj_No + cloth->size()];
	Matrix<double, 3, 4>* A = tetrahedron->data()[tet_obj_No].mesh_struct.A.data();
	...
	for (int i = start; i < end; ++i) {
		if (!is_tet_arap_grad_compute[prefix_sum_start + tet_involved[i]]) {
			...
			memcpy(store_tet_arap_grad.data() + 12 * (prefix_sum_start + tet_involved[i]), grad.data(), 96);//
		}
	}
}
...
void XPBD_IPC::tetGradForColorCollision(int thread_No, unsigned int color_No)
...
void XPBD_IPC::tetGradForColorCollisionNeighbor(int thread_No, unsigned int color_No)
{
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		setCollisionPairTetNeighborGrad(i, collision.tet_involved_in_collision_start_per_thread[obj_No][color_group_index][thread_No],
			collision.tet_involved_in_collision_start_per_thread[obj_No][color_group_index][thread_No + 1], collision.tet_involved_in_collision[obj_No][color_group_index].data(),
			prefix_sum_of_every_tet_index[i]);
	}
}
...
double XPBD_IPC::computeBarrierEnergy()
{
	...
	energy += computeEECollisionEnergy(collision.edge_edge_pair_by_edge, collision.edge_edge_pair_number_record, vertex_position.data(),
		vertex_position.data(), collision.close_ee_pair_num, edge_vertices.data(), edge_vertices.data(),true);
	...
	if (has_collider) {
		...
		energy += computeEECollisionEnergy(collision.edge_edge_collider_pair_by_edge, collision.edge_edge_collider_pair_num_record, vertex_position.data(),
			vertex_position_collider.data(), collision.close_ee_collider_pair_num, edge_vertices.data(), collider_edge_vertices.data(),false);
	}
...
void XPBD_IPC::solveNewtonCD_tetBlock()
{
	...
	for (int i = 0; i < total_thread_num; ++i) {
		newtonCDTetBlockAGroupCollision(i, max_tet_color_num - 1);
	}
...
void XPBD_IPC::newtonCDTetBlockAGroupCollision(int thread_No, int color)
{
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		for (int k = start; k < end; ++k) {
			...
			solveTetBlockCollision(vertex_pos, stiffness, sub_time_step, tet_indices[i], mass, A, tet_neightbor_tet[j],
				...
				vertex_pos_record, vertex_pos_num_record);
			//
			//
		}
		if (perform_collision) {
			//tet that involved in collision (which exclude the original in last color)
			end = collision.tet_involved_in_collision_start_per_thread[obj_No][color_group_index][thread_No + 1];
			start = collision.tet_involved_in_collision_start_per_thread[obj_No][color_group_index][thread_No];
			tet_group = collision.tet_involved_in_collision[obj_No][color_group_index].data();
	...
	if (perform_collision) {
		...
		for (int k = start; k < end; k += 5) {
			obj_No_0 = collision.ee_pair_compressed_record[k];
		...
		for (int k = start; k < end; ++k) {
			if (!is_tet_involved_in_collision[k]) {
				...
				solveTetBlock(vertex_pos, stiffness, sub_time_step, mass, A, tet_neightbor_tet[j],
					...
					obj_No, unfixed_actual_vertex_index[j].data(), hessian_record, grad_record);
				//
				//
			}
		}
	...
	for (auto i = edge_of_a_tet->begin(); i < edge_of_a_tet->end(); i+=2) {
		...
		if (has_collider) {
			collision.EECollisionTimeOneEdgeAll(initial_vertex_position[*i][*edge_].data(),
				...
				vertex_position_collider.data(), vertex_position_collider.data(), collider_edge_vertices.data(),2);
		}
	}
	...
	int* triangle_;		unsigned int* edge_;
		...
		for (auto i = edge_of_a_tet->begin(); i < edge_of_a_tet->end(); ++i) {
			...
			collision.EECollisionTimeOneEdgeAll(initial_vertex_position[*edge_].data(),
				...
				this->record_collision_free_vertex_position_address.data(), vertex_position.data(), edge_vertices.data(),3);
			if (has_collider) {
				collision.EECollisionTimeOneEdgeAll(initial_vertex_position[*edge_].data(),
					initial_vertex_position[*(edge_ + 1)].data(),
					current_vertex_position[*edge_].data(),
	...
	std::array<double, 3>* vertex_position)
{
	...
	if (has_collider) {
		for (int i = 0; i < unfixed_tet_vertex_num; ++i) {
			...
			getVTCollisionHessainForTetFromRecord(Hessian, grad, 
				vertex_position[tet_actual_unfixed_vertex_indices[i]].data(), collision_stiffness,
	...
	if (has_collider) {
		for (auto i = triangle_of_a_tet->begin(); i < triangle_of_a_tet->end(); ++i) {
			triangle_ = triangle_indices[obj_No][*i].data();
			getTVCollisionHessainForTetFromRecord(Hessian, grad, vertex_position[triangle_[0]].data(), vertex_position[triangle_[1]].data(),
	...
	if (has_collider) {
		for (auto i = edge_of_a_tet->begin(); i < edge_of_a_tet->end(); ++i) {
			...
			getEECollisionHessainForTetFromRecord(Hessian, grad, 
				vertex_position[*edge_].data(), vertex_position[*(edge_+1)].data(), collision_stiffness, rest_edge_length[obj_No][*i],
				obj_No, *i, edge_,
				collision.edge_edge_pair_by_edge[obj_No] + collision.close_ee_pair_num * (*i),
				collision.edge_edge_pair_number_record[obj_No][*i],
...
void XPBD_IPC::getTVCollisionHessainForTetFromRecord(MatrixXd& Hessian, VectorXd& grad, double* t0, double* t1, double* t2, double stiffness,
	...
	if (has_collider) {
		for (int i = 0; i < collider_num; i += 2) {
			if ((*tv_collider_hessian_record_index) != 0) {
				...
				//second_order_constraint.computeVTBarrierGradientHessianTest(k1, g1, vertex_position_collider[TV_collider[i]][TV_collider[i + 1]].data(),
				//	t0, t1, t2,
				//	collision.d_hat_2, triangle_vertex_order_in_tet, stiffness, tv_collider_hessian_record_index + 1,*tv_collider_hessian_record_index);
			}
...
void XPBD_IPC::getVTCollisionHessainForTetFromRecord(MatrixXd& Hessian, VectorXd& grad, double* vertex_pos_, double stiffness,
	...
	if (has_collider) {
		...
		for (int i = 0; i < num_collider; i += 2) {
			if ((*ee_collider_hessian_record_index) != 0) {
				...
				setTetHessianFromBarrierHessian(k0, g0.data(), ee_collider_hessian_record, ee_collider_grad_record, vertex_order_in_tet,
				...
				second_order_constraint.computeEEBarrierGradientHessianTest(ea0,ea1,
					vertex_position_collider[EE_collider[i]][*edge_vertex].data(), vertex_position_collider[EE_collider[i]][*(edge_vertex + 1)].data(),
					k1,g1, vertex_order_in_tet, stiffness, collision.d_hat_2, rest_edge,rest_edge_length_collider[EE_collider[i]][EE_collider[i+1]],
					*ee_collider_hessian_record_index, ee_collider_hessian_record_index + 1,0, obj_No,edge_index, EE_collider[i], EE_collider[i+1]);
				if (k0 != k1) {
					std::cout << "eec hessaian is not right " << std::endl;
				...
				if (g0 != g1) {
					std::cout << "eec grad is not right " << std::endl;
...
void XPBD_IPC::solveBlockWithPair(unsigned int vertex_obj_no, unsigned int triangle_obj_No,
	...
	int** record_vertex_num, int* unfixed_pair_vertex_index, int unfixed_num)
{
	...
	getHessianForCollisionBlock(Hessian, grad, vertex_obj_no, triangle_obj_No, triangle_around_vertex, triangle_around_triangle,
		edge_around_vertex, edge_around_triangle,
		tet_around_vertex, tet_around_triangle,
		unfixed_pair_vertex_index, unfixed_num, &around_triangle, &around_edge, &around_tet);
	...
	int** record_vertex_num)
{
	...
	solveBlockWithPair(vertex_obj_no, triangle_obj_No, 
		dt, triangle_around_vertex, triangle_around_triangle, edge_around_vertex, edge_around_triangle,
		tet_around_vertex, tet_around_triangle, record_vertex_position, record_vertex_num, unfixed_pair_vertex_index, unfixed_num);
}
	...
	int** record_vertex_num)
{
	...
	solveBlockWithPair(obj_No_0, obj_No_1, 
		dt, triangle_around_0, triangle_around_1, edge_around_0, edge_around_1,
		tet_around_0, tet_around_1, record_vertex_position, record_vertex_num, unfixed_pair_vertex_index, unfixed_num);
}
	...
	int unfixed_vertex_num)
{
	...
	if (has_collider) {
		for (auto i = triangles->begin(); i < triangles->end(); i += 2) {
			triangle_ = triangle_indices[*i][*(i + 1)].data();
			getTVCollisionHessainForPairFromRecord(Hessian, grad,
				collision.triangle_vertex_pair_by_triangle[*i] + collision.close_tv_pair_num * (*(i + 1)),
				collision.triangle_vertex_pair_num_record[*i][*(i + 1)], *i,
	...
	int* record_vertex_num)//
{
	...
	second_order_constraint.solveCD_ARAP_block_fromRecord(Hessian, grad,
	unfixed_tet_vertex_index, unfixed_vertex_num, hessian_record + 16 * tet_index, grad_record + 12* tet_index);
	...
	double* hessian_record, double* grad_record)//
{
	...
	if (perform_collision) {
		getFloorHessian(Hessian, grad, tet_actual_unfixed_vertex_indices, unfixed_vertex_num, collision.d_hat_2,
			vertex_index_surface[obj_No], vertex_position, collision_stiffness);
	}
