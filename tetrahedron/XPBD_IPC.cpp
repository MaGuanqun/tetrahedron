...
void 	XPBD_IPC::tetHessian()
{
	...
	std::unordered_map<std::array<unsigned int, 2>, double, pair_hash>* hessian;
	...
	for (int i = 0; i < tetrahedron->size(); ++i) {
		...
		for (auto j = 0; j < tet_around_tet_color_group_end; ++j) {
			...
			for (int k = 0; k < 4; ++k) {
				for (int m = 0; m < 4; ++m) {
					address = hessian->find(std::array{ (unsigned int)tet_vertex[k], (unsigned int)tet_vertex[i] });
					if (address != hessian->end()) {
						address->second += Hessian.data()[(m << 2) + k];
					}
					else {
						hessian->emplace(std::array{ (unsigned int)tet_vertex[k], (unsigned int)tet_vertex[i] }, Hessian.data()[(m << 2) + k]);
					}
				}
			}
		}
		...
		for (int k = start; k < end; ++k) {
			//if (!is_tet_involved_in_collision[k]) {
				...
				solveTetBlock(vertex_pos, stiffness, sub_time_step, mass, A, tet_neightbor_tet[j],
					...
					&(triangle_of_a_tet[j]), &(edge_of_a_tet[j]), vertex_index_on_surface);
				//
				//
			//}
		}
	...
	int* triangle_;		unsigned int* edge_;
		...
		for (int i = 0; i < unfixed_tet_vertex_num; ++i) {
			...
			collision.VTCollisionTimeOneVertex(initial_vertex_position[tet_actual_unfixed_vertex_indices[i]].data(),
				...
				this->record_collision_free_vertex_position_address.data(), vertex_position.data(), triangle_indices.data(),false, tet_actual_unfixed_vertex_indices[i]);
			if (has_collider) {
				collision.VTCollisionTimeOneVertex(initial_vertex_position[tet_actual_unfixed_vertex_indices[i]].data(),
					current_vertex_position[tet_actual_unfixed_vertex_indices[i]].data(), collision_time,
