#include"set_tetrahedron_anchor.h"
#include"global.h"

SetTetrahedronAnchor::SetTetrahedronAnchor()
{
	position.resize(4);
...
void SetTetrahedronAnchor::setCorner(double* screen_pos, bool pre_press_state, std::vector<Tetrahedron>& tetrahedron, Camera* camera, 
	std::vector<bool>& hide)
{
	if (!pre_press_state) {
		...
		for (int i = 0; i < tetrahedron.size(); ++i) {
			if (!hide[i]) {
				all_select_num += tetrahedron[i].mesh_struct.anchor_vertex.size();
			}
		}
...
void SetTetrahedronAnchor::setClipSpaceRange(double* screen_pos)
{
...
void SetTetrahedronAnchor::transferTo3D(double* screen_pos, float* point)
{
...
void SetTetrahedronAnchor::setPosition()
{
	float y[2], z[2];
...
void SetTetrahedronAnchor::genBuffer()
{
...
void SetTetrahedronAnchor::setBufferData()
{
...
void SetTetrahedronAnchor::draw()
...
void SetTetrahedronAnchor::findAllSelectVertex(std::vector<std::array<double,3>>& vertex_for_render, Camera* camera, std::vector<int>& vertex_index)
{
...
void SetTetrahedronAnchor::findSurfaceVertex(std::vector<std::array<double, 3>>& vertex_for_render, Camera* camera, std::vector<int>& vertex_index, std::vector<bool>& is_surface)
{
