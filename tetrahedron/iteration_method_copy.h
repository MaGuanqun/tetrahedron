...
private:
	...
	template <class T>
	void solveByJacobiPerThread(int thread_id, Matrix<T, -1, 1>& u, Matrix<T, -1, 1>& b, SparseMatrix<T, RowMajor>& system_matrix, SparseMatrix<T, RowMajor>& R_Jacobi, Matrix<T, -1, 1>& global_diagonal_inv, Matrix<T, -1, 1>& ground_truth, std::vector<double>& relative_error)
	{
