...
private:
	...
	void saveTetrohedronIndex(std::vector<std::vector<std::string>>& result, OriMesh& mesh);
	void setNodePath(std::string& node_path, std::string& ele_path);
	void saveTetrohedronVertex(std::vector<std::vector<std::string>>& result, OriMesh& mesh);
};
