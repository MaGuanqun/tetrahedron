...
void ReadEle::load(const char* name, OriMesh& mesh)
{
	...
	tem_string.clear();
	saveTetrohedronIndex(read_string, mesh);
	read_string.clear();
	loadFile(node_path.c_str(), tem_string);
	splitting(tem_string, read_string);	
	tem_string.clear();
	saveTetrohedronVertex(read_string, mesh);
...
void ReadEle::saveTetrohedronIndex(std::vector<std::vector<std::string>>& result, OriMesh& mesh)
{
	bool start_from_zero=false;
	int tetrohedron_num = stoi(result[0][0]);
	mesh.indices.reserve(4* tetrohedron_num);
...
void ReadEle::saveTetrohedronVertex(std::vector<std::vector<std::string>>& result, OriMesh& mesh)
{
	int tetrohedron_vertex_num = stoi(result[0][0]);
	mesh.vertices.reserve(3 * tetrohedron_vertex_num);
	for (int i = 1; i < result.size()-1; ++i) {
	...
	if (mesh.vertices.size() != tetrohedron_vertex_num) {
