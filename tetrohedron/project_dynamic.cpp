...
void ProjectDynamic::setIndexPerThread()
{
	...
	tetrohedron_per_thread_begin.resize(total_thread_num + 1);
	if (total_tetrohedron_num > 0) {
		(*tetrohedron)[0].mesh_struct.arrangeIndex(total_thread_num, total_tetrohedron_num, tetrohedron_per_thread_begin);
	}
	tetrohedron_dimension_per_thread.resize(total_thread_num);
	total_num = total_tetrohedron_num * 3;
	for (int i = 0; i < total_num; ++i) {
		tetrohedron_dimension_per_thread[i % total_thread_num].push_back(i);
	}
}
...
void ProjectDynamic::setForTetrohedronPD(std::vector<Tetrohedron>* tetrohedron)
{
	...
	for (int i = 0; i < total_tetrohedron_num; ++i) {
		tetrohedron_sys_size[i] = (*tetrohedron)[i].mesh_struct.vertex_position.size();
	}
}
...
void ProjectDynamic::reset()
{
	...
	for (int j = 0; j < total_tetrohedron_num; ++j) {
		...
		for (int i = 0; i < 3; ++i) {
			tetrohedron_u_[j][i] = tetrohedron_u[j][i];
			tetrohedron_v[j][i].setZero();
			tetrohedron_v_[j][i].setZero();
			tetrohedron_acceleration[j][i].setZero();
		}
	}
...
void ProjectDynamic::initial()
{
	...
	tetrohedron_global_mat = initial_tetrohedron_global_mat;
...
void ProjectDynamic::updateModelPosition()
{
	...
	for (int j = 0; j < total_tetrohedron_num; ++j) {
		for (int i = 0; i < tetrohedron_sys_size[j]; ++i) {
			(*tetrohedron)[j].mesh_struct.vertex_position[i][0] = tetrohedron_u[j][0].data()[i];
			(*tetrohedron)[j].mesh_struct.vertex_position[i][1] = tetrohedron_u[j][1].data()[i];
			(*tetrohedron)[j].mesh_struct.vertex_position[i][2] = tetrohedron_u[j][2].data()[i];
		}
	}
}
...
{
	PDClothPredict();
	PDTetrohedronPredict();

	updateModelPosition();
}

void ProjectDynamic::PDTetrohedronPredict()
{
...
{	
	...
	for (int i = tetrohedron_per_thread_begin[thread_id]; i < tetrohedron_per_thread_begin[thread_id + 1]; ++i) {
		tetrohedron_llt[i].factorize(cloth_global_mat[i]);
	}
}
